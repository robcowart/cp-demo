...
clean_demo_env()
{
  ...
  echo "CLEAN=true -> deleting existing certificates and local Connect and Audit Log Consumer Docker images generated by cp-demo"
  ...
  # Remove existing audit log consumer image
  docker rmi -f localbuild/audit-log-consumer:latest
}
...
build_audit_log_consumer_image()
{
  echo
  echo "Building custom Docker image to consume audit logs."
  
  DOCKERFILE="${DIR}/../Dockerfile-audit"

  echo "docker build  -f $DOCKERFILE -t localbuild/audit-log-consumer:latest ."
  docker build  -f $DOCKERFILE -t localbuild/audit-log-consumer:latest . || {
    echo "ERROR: Docker image build failed. Please troubleshoot and try again. For troubleshooting instructions see https://docs.confluent.io/current/tutorials/cp-demo/docs/index.html#troubleshooting"
    exit 1
  }
}
